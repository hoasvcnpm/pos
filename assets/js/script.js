(function($) {
  ...
  if( account() ) {
    ...
    $(document).ready(function($){
      ...
      $('.btn-cart-payment').on('click', function(){
        $('.section-cart')
          .removeClass('step-cart')
          .addClass('step-payment')
          .find('.quality').attr('disabled','disabled');
      });
      ...
      $('.btn-cart-complete').on('click', function(){
        if( confirm('Bạn có chắc muốn hoàn thành đơn hàng của bàn số ' + main_table_number + ' không?') ) {
          var list_cart = get_items('cart', { table: main_table_number, completed:0 }),
            t = get_time(),
            d = get_datetime();
          ...
          $('.card').eq( main_table_number - 1 ).removeClass('has-cart')
            .find('.fa-shopping-cart').text( '' );
  ...
  function number_format( number, after, before ) 
  {
    ...
    }), value = formatter.format( number ).replace('$',''), text = [];
  ...
  {
    var list = [], key = '', item = {}, text = prefix + name;
    ...
    if( typeof where == 'object' ) {
      ...
      for( i in keys ){
        var key = keys[i], value = where[key];
  ...
  function table_stories() {
    $('.table-in-store').each(function(){
      ...
      $('.card', p).each(function(i) {
        var e = $(this),
          number = i+1,
          list_cart = get_items('cart', {table: number, completed:0 });
      ...
      $('.btn-add-card', p).on('click', function(e) {
        ...
        var card = $(html).removeClass('hide'), total = $('.col-card', p).length+1;
      ...
      p.on('click', '.card', function(){
        var card = $(this), number = parseInt( $('.card-title', card).text() );
  ...
  function table_cart() {
    ...
    $('.table-cart').each(function(){
      var p = $(this),
        ...
        list_cart = get_items('cart', {table:main_table_number,completed:0}),
        tbody = $('tbody', p).empty();
      
      if( list_menu.length == 0 ) {
        $('#modal-noti').each(function(){
      ...
      function update_total()
      {
        ...
        $('.table-number-' + main_table_number ).find('.fa-shopping-cart').text(n);
      ...
      list_menu.forEach( item => {
        var cart = list_cart.find( o => o.menu_id == item.id ), id = '';
        ...
        var td = '<th class="name" scope="row">'+ item.name +'</th>',
          ...
          inputs = [
            ...
            '<i class="fa fa-minus input-group-text icon-quality"></i>',
            '<input type="number" class="form-control quality" value="'+ item.quality +'">',
            '<i class="fa fa-plus input-group-text icon-quality"></i>',
            '</div>'
          ];
  ...
  function table_cart_back() {
    $('.section-cart')
      .removeClass('step-payment')
      .addClass('step-cart')
      .find('.quality').removeAttr('disabled');
  }
  ...
  function login( value ) {
    if( $('body').hasClass('login-page') ) {
      ...
      if( value == null ) {
        $('.btn-register').removeClass('hide').on('click',function() {
          submit_login('register');
        });
      } else {
        $('.btn-login').removeClass('hide').on('click',function() {
          submit_login('login');
        });
      }
    } else {
